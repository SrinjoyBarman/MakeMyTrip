...
public static WebDriver getLocalWebdriver(String browsername,String portId) {
...
public static Properties propertySet() {
	if(prop==null) {
		prop= new Properties();
		try {
			FileInputStream fis= new FileInputStream(System.getProperty("user.dir")+"//mmt.properties");
			try {
				prop.load(fis);
			} catch (IOException e) {
				e.printStackTrace();
